...
public class UploadController {
	...
	// 썸네일 이미지 생성을 할 것인지 안 할 것인지에 대한 판단하는 메소드(사용자가 업로드 한 파일이 이미지이면 생성, 그렇지 않으면 생성
	// 안함)
	...
	public ResponseEntity<ArrayList<AttachFileDTO>> uploadAjaxAction(MultipartFile[] uploadFile) {
		// AttachFileDTO에 저장되는 값이 여러파일에 대한 값이면 배열로 처리가 되어야 하므로 ArrayList타입이 되어야 함. 배열에
		// 저장하여 여러개가 나올 수 있게 함.
		ArrayList<AttachFileDTO> list = new ArrayList<>();

		// 파일 업로드할 경로 지정
		String uploadFolder = "C:\\upload";// 날짜가 없는 것

		// 폴더 생성 (기존 폴더, 현재 폴더)위치를 결합
		File uploadPath = new File(uploadFolder, getFolder());// 날짜가 있는 것 //실제 업로드 경로
		System.out.println("uploadAjaxAction uploadPath=" + uploadPath);
		...
		for (MultipartFile multipartFile : uploadFile) {// uploadFile배열에 변수를 하나 만들어 값을 multipartFile에 저장하여 출력
			...
			// 실제 파일명(uploadFileName)을 AttachFileDTO클래스(attachdto)에 fileName에
			// 저장(setFileName)
			...
			try {
				...
				// 실제 업로드 경로(uploadFolderPath)을 AttachFileDTO클래스(attachdto)에 uploadPath에
				// 저장(setUploadPath)
				...
				if (checkImage(saveFile)) {// 이미지 파일이면
					...
					FileOutputStream thumnail = new FileOutputStream(new File(uploadPath, "s_" + uploadFileName));// FileOutput
																													// 파일을
																													// 밖으로
																													// 뺸다
			...
			// 통신상태가 정상적(HttpStatus.OK)이면 ArrayList(list)에 저장되어 있는 값을 웹브라우저(uploadAjax.js에
			// 있는 ajax에 success)에 보내라
	...
	@GetMapping("display")
	public ResponseEntity<byte[]> getFile(String fileName) {// getfile()은 문자열로 파일의 경로가 포함된 fileName을 매개변수 받고
															// byte[](이진수)를 전송
		...
		// byte[]로 이미지 파일의 데이터를 전송할 때 브라우저에 보내는 MIME타입이 파일의 종류(jpg, png, xls, ppt...)에
		// 따라서 달라진다.
		// 이 부분을 해결하기 위해서 probeContentType()을 이용해서 MIME타입 데이터를 Http의 헤더 메세지에 포함할 수 있도록
		// 처리
		try {
	...
	// consumes : 들어오는 데이터 타입 정의(생략가능) //produces : 반환하는 데이터 타입 정의(생략가능)//*생략을 하게
	// 되면, 웹브라우저가 알아서 타입을 판단(내가 원하는 타입이 아닐 수도 있음)*
	// 웹브라우저가 '이 파일은 download해야 하는 파일입니다.' 라는 것을 인지할 수 있도록 반환이 되어야 한다. 그러기 위해서는
	// MediaType.APPLICATION_OCTET_STREAM_VALUE 타입으로 반환데이터 타입을 선언한다.
